...
namespace org.neurul.Brain.Port.Adapter.IO.Persistence.Events.SQLite.Test.EventStoreFixture.given
{
    ...
    public class When_saving_event 
    {
        public class When_store_is_empty : ProperlyConstructedContext
        {
            ...
            {
                Assert.StartsWith("org.neurul.Brain.Domain.Model", (await this.sut.GetLastEvent(this.guid)).ToEventInfo(this.serializer).TypeName);
            }

            [Fact]
            public async Task Should_contain_correct_typename()
            {
                Assert.StartsWith("org.neurul.Brain.Domain.Model.Neurons.NeuronCreated", (await this.sut.GetLastEvent(this.guid)).ToEventInfo(this.serializer).TypeName);
            }
